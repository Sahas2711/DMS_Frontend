...
import { fetchDestinations, fetchTours, resolveMediaUrl } from '../services/api/cms';
...
const Home = () => {
    ...
    const [popularTours, setPopularTours] = useState(POPULAR_TOURS);
    const [domesticDestinations, setDomesticDestinations] = useState(DOMESTIC_DESTINATIONS);
    const [internationalDestinations, setInternationalDestinations] = useState(INTERNATIONAL_DESTINATIONS);

    // Live CMS data — the temp_ui static arrays above are the offline fallback
    // used when the backend is unreachable, so the page always renders.
    useEffect(() => {
        let cancelled = false;
        ...
        Promise.allSettled([
        ...
        ]).then(([destsRes, toursRes]) => {
            if (cancelled) return;

            const apiDestinations = (destsRes.status === 'fulfilled' ? destsRes.value?.items : []) || [];
            ...
            if (apiDestinations.length) {
                ...
                setDomesticDestinations(
                    cards.map((d) => ({ title: d.title, tours: 'JOURNEYS AVAILABLE', image: d.image })).slice(0, 3)
                );
                setInternationalDestinations(
                    cards.slice(3, 6).map((d) => ({ title: d.title, tours: 'JOURNEYS AVAILABLE', image: d.image }))
                );
            }
            ...
            if (apiTours.length) {
                ...
                setPopularTours([...tours, ...POPULAR_TOURS].slice(0, 5));
            }
        });
