...
const Trip = () => {
    ...
    const [matchStatus, setMatchStatus] = useState('idle'); // idle|loading|success|error
    ...
    const handleMatch = async () => {
        ...
        try {
            const data = await fetchRoutes({ pageSize: 20, sort: 'display_order' });
            setMatches(data.items || []);
            setMatchStatus('success');
        } catch (err) {
            setMatchStatus('error');
            setMatchError(errorMessage(err, 'Could not load itinerary ideas right now.'));
        }
        notificationTimer.current = setTimeout(() => setMatchedNotification(false), 3000);
    };

    const showNotification = matchStatus === 'loading' || matchedNotification;
    ...
    return (
        <div className="w-full bg-[#FFFFFF]">
            ...
            <section className="w-full bg-[#0E1C37] py-6 md:py-8 px-6 md:px-12 lg:px-20 xl:px-32 flex justify-center border-t border-white/5">
                <div className="max-w-7xl w-full flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    ...
                    <div className="flex flex-wrap items-center gap-3 relative">
                        ...
                        {showNotification && (
                            <div
                                className={`absolute -bottom-10 right-0 text-white text-xs px-4 py-1.5 rounded-full shadow-lg animate-fade-in ${
                                    matchStatus === 'error' ? 'bg-red-500' : 'bg-emerald-500'
                                }`}
                                role={matchStatus === 'error' ? 'alert' : 'status'}
                            >
                                {matchStatus === 'loading' && 'Finding personalized itineraries for you...'}
                                {matchStatus === 'success' && `${matches.length} itinerary ${matches.length === 1 ? 'idea' : 'ideas'} matched for your trip.`}
            ...
            {/* Matched Itineraries (from the published routes API) */}
            {matchStatus === 'success' && matches.length > 0 && (
                <section className="w-full bg-[#0E1C37] py-12 md:py-16 px-6 md:px-12 lg:px-20 xl:px-32 flex justify-center border-t border-white/5">
                    <div className="max-w-7xl w-full">
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
                            <div>
                                <span className="text-[10px] font-bold tracking-[0.2em] text-gray-400 uppercase mb-1 block">
                                    MATCHED FOR YOUR TRIP
                                </span>
                                <h2 className="text-white text-xl sm:text-2xl md:text-[26px] font-serif font-bold">
                                    Itinerary ideas from our Hanoi designers
                                </h2>
                            </div>
                            <Link
                                to="/tours"
                                className="bg-white/10 hover:bg-white/20 text-white font-medium text-xs sm:text-sm px-5 py-2.5 rounded-full transition-all border border-white/20 w-fit whitespace-nowrap"
                            >
                                See all experiences
                            </Link>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 md:gap-6">
                            {matches.map((route) => {
                                const cover = resolveMediaUrl(route?.hero_media?.url || route?.origin_destination?.hero_media?.url);
                                const routeLabel = route.origin_destination?.name && route.end_destination?.name
                                    ? `${route.origin_destination.name} → ${route.end_destination.name}`
                                    : (route.origin_destination?.name || route.slug);
                                return (
                                    <Link
                                        key={route.public_id}
                                        to="/tours"
                                        className="group bg-white rounded-2xl overflow-hidden border border-white/10 shadow-md hover:shadow-xl transition-all duration-300 flex flex-col"
                                    >
                                        {cover && (
                                            <div className="relative aspect-[16/10] overflow-hidden">
                                                <img
                                                    src={cover}
                                                    alt={route.hero_media?.alt_text || routeLabel}
                                        ...
                                        <div className="p-5 sm:p-6 flex flex-col flex-grow">
                                            <div className="flex items-center justify-between mb-2">
                                                <span className="text-[10px] font-bold tracking-wider text-navy uppercase">{routeLabel}</span>
                                                {route.duration_days && (
                                                    <span className="bg-[#F3F4F6] text-gray-700 text-[10px] font-semibold px-2.5 py-1 rounded-full">
                                                        {route.duration_days} day{route.duration_days === 1 ? '' : 's'}
                                                    </span>
                                                )}
                                            </div>
                                            <h3 className="text-navy font-serif font-bold text-lg mb-2 group-hover:text-gold transition-colors">
                                                {route.title}
                                            </h3>
                                            {route.description && (
                                                <p className="text-steel text-xs sm:text-sm leading-relaxed line-clamp-3">
                                                    {route.description}
                                                </p>
                                            )}
                                            <span className="mt-4 text-navy font-bold text-xs sm:text-sm flex items-center gap-1 group-hover:text-gold transition-colors">
                                                Explore this idea →<span className="sr-only">{route.title}</span>
                                            </span>
                                        </div>
                                    </Link>
                                );
